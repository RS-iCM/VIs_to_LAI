        ...
        for j, vi in enumerate(f_vis):
            ...
            with open(file_path, 'rb') as file:
                ...
                if data.shape[0] != n_pixels:
                    raise ValueError(f"Expected number of pixels: {n_pixels}, \
                    but got: {data.shape[0]} from file: {file_path}")
...
### -- Function to predict LAI using ML(ET) model
def predict_LAI_using_ML(data, pkl_filename):
    #pkl_filename = '/models/pickle_extra_trees_Rice.pkl'  # Load from file
    with open(pkl_filename, 'rb') as file:
        pickle_model = pickle.load(file)
    try:
        log10_log_DOY = np.log10(np.log(data[:, 0]))  
        processed_data = np.column_stack((log10_log_DOY, data[:, 1:5]))
        
        # Predict LAI using the loaded model
        Ypredict = pickle_model.predict(processed_data)
        SLAI = Ypredict  # Use predictions directly - model outputs LAI values directly

        return SLAI
        
    except NotFittedError as e:
        print("Model is not fitted. Please check if the model has been trained correctly before saving.")
        raise e
    except ValueError as e:
        print("Invalid value encountered:", e)
        raise e

### -- Function to estimate LAI using VIs based on ML --
def ML_VIs_to_LAI(n_obs,n_doys,n_pixels,sat_opt,start_doy,rs_data,paddyFields_data,plantingDate_data,cg_period,pkl_filename):
    ...
    num = 0
    debug_stats = {'total_pixels': n_pixels, 'rice_pixels': 0, 'sufficient_data': 0, 'valid_dates': 0, 'ml_predictions': 0, 'final_stored': 0}
    
    for i in range(n_pixels):
        VIs_data = np.full((5, n_doys), np.nan)

        count = 0
        # Collecting and preparing VI data
        for j in range(0, n_obs, 8):  # Process data in 8-day intervals
            #yrNum = rs_data[j, 0] if sat_opt == 1 else start_doy + j
            if sat_opt == 1:
                yrNum = n_doys[j]
            elif sat_opt == 2:
                yrNum = start_doy + j
            ...
            elif rs_data[1,j,i] > 0 and paddyFields_data[i] == 1 and \
            yrNum > plantingDate_data[i] + 10 and yrNum < plantingDate_data[i] + cg_period:              
                VIs_data[:, count] = [yrNum] + list(rs_data[1:5,j,i])
                
                count += 1  # counte observed data
        
        # Debug: Track rice field pixels
        if paddyFields_data[i] == 1:
            debug_stats['rice_pixels'] += 1
            
        # Trim and check data validity
        VIs_data = VIs_data[:, :count]
        VIs_data = VIs_data.transpose() ### Transpose the array to shape (5, :)

        # Debug: Track pixels with sufficient data
        if count > 5:
            debug_stats['sufficient_data'] += 1
            
        # Debug: Track pixels with valid date ranges
        if count > 5 and VIs_data[0, 0] > plantingDate_data[i]+10 and \
        VIs_data[count-1, 0] < plantingDate_data[i] + cg_period:
            debug_stats['valid_dates'] += 1

        if count > 5 and VIs_data[0, 0] > plantingDate_data[i]+10 and \
        VIs_data[count-1, 0] < plantingDate_data[i] + cg_period:
            try:                  
                #SLAI = predict_LAI_using_ML(VIs_data)   ## Calling 'predict_LAI_using_ML' function
                SLAI = predict_LAI_using_ML(VIs_data, pkl_filename)
                debug_stats['ml_predictions'] += 1
            except ValueError:
                continue  # Skip current iteration if invalid values are encountered

            # Applying the model prediction and storing LAI values
            for j in range(0, n_obs, 8):
                yrNum = rs_data[j, 0] if sat_opt == 1 else start_doy + j
                for k in range(count):
                    if VIs_data[k, 0] == yrNum and 0.1 < SLAI[k] < 10.0:
                        LAI_data[j, i] = SLAI[k]
                        debug_stats['final_stored'] += 1
        
        num += 1
        if num % 500 == 0: print(f'Run #{num}')

    num2 = 0
    for i in range(0, n_obs, 8):
        #yrNum = rs_data[i, 0] if sat_opt == 1 else start_doy + j
        if sat_opt == 1:
            yrNum = n_doys[i]
        elif sat_opt == 2:
            yrNum = start_doy + i
        ...
        if yrNum >= start_doy:
            sub_arr_mMLAI[0, num2] = yrNum   ### 
            sub_arr_mMLAI[1, num2] = mMLAI
            sub_arr_mMLAI[2, num2] = stdMLAI
            
        sub_OLAI_data[num2, :] =  LAI_data[i, :] 
        num2 += 1
    
    # Print debugging statistics
    """print("=== ML Processing Debug Statistics ===")
...
def main(cwd, paddy_data_dir, output_dir, map_dir, IO_file_path, pkl_filename):
    """cwd = os.getcwd() # get the current directory
    paddy_data_dir = cwd + '/paddy_data/'
    output_dir = cwd + '/py_LAI_ET/'    
    ...
    if split_op == 0:
        ...
        # calling 'ML_VIs_to_LAI' function
        sub_OLAI_data, sub_arr_mMLAI = ML_VIs_to_LAI(n_obs,n_doys,n_pixels,sat_opt,start_doy,rs_data,
                                         sub_paddyFields,sub_plantingDate, cg_period, pkl_filename)
    ...
    if split_op == 1:   
        ...
        for isplit in range(nsplits + 1):
            ...
            # calling 'read_spatial_VIs' function
            sub_rs_data = read_spatial_VIs(data_dir,sat_opt,area_n,yr, 
                                       list_doys,f_vis,n_obs,npixels, data_start)
    
            # calling 'process_paddyFields_n_plantingDate_data' function
            sub_paddyFields,sub_plantingDate = process_paddyFields_n_plantingDate_data(paddy_data_dir,
                                                                            yr,area_n,npixels,data_start)
    
            # calling 'ML_VIs_to_LAI' function
            sub_OLAI_data,sub_arr_mMLAI = ML_VIs_to_LAI(n_obs,n_doys,npixels,sat_opt,start_doy,sub_rs_data,
                                         sub_paddyFields,sub_plantingDate,cg_period,pkl_filename)            
